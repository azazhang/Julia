...
# Bayesian cointegration testing

@model coint(y,x, ::Type{TV}=Vector{Float64}) where {TV} =begin
    yt = y[2:end]
    yt1 = y[1:(end-1)]
    xt = x[2:end]
    xt1 = x[1:(end-1)]
    ...
    # likelihood
    #	y_t=α* + βx_t+ ϕy_(t-1)- ϕβx_(t-1) + u_t
    mu = alpha .+ beta.*xt .+ phi.*yt1 - phi.*beta.*xt1
    for t in 1:n
        yt[t] ~ Normal(mu[t], sig)
    end
end


@model nonid_coint(y,x, ::Type{TV}=Vector{Float64}) where {TV} =begin
    yt = y[2:end]
    yt1 = y[1:(end-1)]
    xt = x[2:end]
    xt1 = x[1:(end-1)]
    ...
    # likelihood
    # y_t=(λ+α-αϕ)+βx_t+ϕy_(t-1)-ϕβx_(t-1)+u_t
    mu = lambda .+ alpha .- alpha.*phi .+ beta.*xt .+ phi.*yt1 - phi.*beta.*xt1
    for t in 1:n
        yt[t] ~ Normal(mu[t], sig)
    end
end
...
@model ecm_two_step(dy,dx, yt, xt, yt1, xt1, dyt1, dxt1, ::Type{TV}=Vector{Float64}) where {TV} =begin
    n = length(yt)
    xx = [ones(length(xt)) xt]
    blinreg, b2se, tstats, pvals, Rsq, sigma2_hat, cril, criu = linreg(xx,yt)
    xx1 = [ones(length(xt)) xt1]
    ecm =  yt1 .- xx*blinreg
    # priors
    lambda ~ Normal(0,10)
    delta ~ Normal(0,10)
    ...
    # likelihood
    mu = alpha .+ phi.*dyt1 + beta.*dxt1 .+ delta.*ecm
    for t in 1:n
        yt[t] ~ Normal(mu[t], sig)
    end
end

@model ecm_one_step(dy,dx, yt, xt, yt1, xt1, dyt1, dxt1, ::Type{TV}=Vector{Float64}) where {TV} =begin
    n = length(yt)
    # priors
    lambda ~ Normal(0,10)
    delta ~ Normal(0,10)
    ...
    # likelihood
    mu = alpha .+ phi.*dyt1 + beta.*dxt1 .+ delta.*yt1 .- delta.*lambda.*xt1
    for t in 1:n
        yt[t] ~ Normal(mu[t], sig)
    end
end
...
# Estimate the model
model = ecm_one_step(dy,dx, yt, xt, yt1, xt1, dyt1, dxt1)
model = ecm_two_step(dy,dx, yt, xt, yt1, xt1, dyt1, dxt1)
model = coint(y, x)
model = arma21(y)
model = arma12(y)
