...
xx = [ones(length(y)) ones(length(y))]
coefnames = ["cnst" "beta"]
...
# MA(1) model
@model two_means(x, ::Type{TV} = Vector{Float64}) where {TV} =begin
    n = length(x)
    s ~ Uniform(0.000001, 5.0)   #InverseGamma(2,3)
    a1 ~ Normal(0, 3.0)
    a2 ~ Normal(0, 3.0)
...
@model one_mean(x, ::Type{TV} = Vector{Float64}) where {TV} =begin
    n = length(x)
    s ~ Uniform(0.000001, 5.0)   #InverseGamma(2,3)
    a ~ Normal(0, 3.0)
    for i in eachindex(x)  # NEED TO DEFINE u!
...
mean(a1)
std(a1)
plot(a1, st=:density, linewidth = 2)

aa = get(cc,:a2)
a2 = Array(aa.a2)
mean(a2)
std(a2)
plot!(a2, st=:density, linewidth = 2)

a = a1 .+ a2
plot(a, st=:density, linewidth = 2)

# from model with one mean
auno = get(cc,:a)
onea = Array(auno.a)
mean(onea)
std(onea)
plot!(onea, st=:density, linewidth = 2)
